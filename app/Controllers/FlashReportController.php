...
final class FlashReportController
{
    ...
    public function create(string $id): never
    {
        ...
        if (!in_array($reason, self::REASONS, true)) {
            Response::error('VALIDATION_ERROR', 'Report reason is invalid', 422, ['reason' => 'Choose a valid report reason']);
        }
        if (mb_strlen($description) > 500) {
            Response::error('VALIDATION_ERROR', 'Report description is too long', 422, ['description' => 'Maximum length is 500 characters']);
        }
        ...
        if ($flash['reporter']['id'] === $userId) {
            Response::error('FORBIDDEN', 'You cannot report your own flash', 403);
        }
        if (!$this->limits->allow('flash:report', (string) $userId, 10, 3600)) {
            Response::error('RATE_LIMITED', 'Too many reports', 429);
        }

        try {
            Response::json($this->reports->create($flashId, $userId, $reason, $description !== '' ? $description : null), 201);
        } catch (\DomainException) {
            Response::error('CONFLICT', 'You have already reported this flash', 409);
        }
    }
